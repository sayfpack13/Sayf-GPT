...
async function* parseJsonStream(readableStream) {
    for await (const line of readLines(readableStream.getReader())) {
        ...
        const trimmedLine = line.slice(line.indexOf('{'), line.lastIndexOf('}') + 1).trim();
    ...
    getSearchEngineResult(user_message, thread_id, (data, error) => {
        ...
        getCHATGPTMessage(search_query, thread_id + 1, settings, chat_history, (data, done, error) => {
            ...
            if (done && !error) {
                ...
                if (chat_history[chat_history.length - 2].internet_content === undefined) {
                    chat_history[chat_history.length - 2].internet_content = search_query
...
export function checkResult(text) {
    ...
    if (text.includes("ai language") || (text.includes("do not")) || (text.includes("apologize")) || (text.includes("cannot") || text.includes("sorry") || text.includes("don't") || text.includes("can't"))) {
        return true
    }
...
export function getIPInformations(callback) {
    fetch(IP_API_URL).then((response) => response.json()).then((data) => {
        fetch(IP_INFORMATIONS_API_URL + "/" + data.ip + "/json").then(response => response.json()).then((data) => {
